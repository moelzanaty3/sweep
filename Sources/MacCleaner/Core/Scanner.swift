...
enum DiskScanner {
            ...
            for path in candidates {
                group.addTask {
                    ...
                    return CleanItem(
                        ...
                        rationale: "Cache written by \(prettyBundleName(name)). The app regenerates it on next launch; your settings and data live elsewhere."
                    )
                }
            }
            var out: [CleanItem] = []
    ...
    private static func rationaleForArtifact(_ name: String) -> String {
        switch name {
        case "node_modules": return "Restored by `npm install` / `pnpm install` / `yarn` in this project. Your source and lockfile are untouched."
        case ".next", ".nuxt", ".svelte-kit", ".turbo", ".parcel-cache": return "Framework build output. Regenerated by the next dev server start or build."
